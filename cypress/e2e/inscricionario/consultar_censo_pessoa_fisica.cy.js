/// <reference types="cypress" />

describe("Teste HGTX CRECI - Consultar Censo - Pessoa Física", () => {
  beforeEach(() => {
    cy.loginCreci();
    cy.intercept("POST", "**/listar_apps_usuario/**", {
      statusCode: 200,
      body: {
        totalRegistros: 9,
        results: [
          {
            aplicativoID: 162,
            nome: "Consultar Censo - Pessoa Física",
            favorito: false,
            icone:
              "https://creci-hgtxcore-api.goutron.com.br/api/v1/image_render/app_ico/162",
            urlDestino: "https://creci-procinscr.goutron.com.br/auth/censo-pf",
            nativoMobile: false,
            disponivel: true,
            indisponivelMensagem: "",
            appShareUrl:
              "https://creci-hgtxcore.goutron.com.br/app-share-redir?token=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..LJ62nqqaENA2Cqi5BN0byADv4u0YoK52No6kiqA4bWk",
          },
          {
            aplicativoID: 158,
            nome: "Gerenciar Inscrições de Estágio",
            favorito: false,
            icone:
              "https://creci-hgtxcore-api.goutron.com.br/api/v1/image_render/app_ico/158",
            urlDestino: "https://creci-procinscr.goutron.com.br/auth/estagios",
            nativoMobile: false,
            disponivel: true,
            indisponivelMensagem: "",
            appShareUrl:
              "https://creci-hgtxcore.goutron.com.br/app-share-redir?token=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..QYpfw_ljP5eUC7HDHrmY_ak23w9stUyVj6JmNnftZKg",
          },
          {
            aplicativoID: 161,
            nome: "Gerenciar Inscrições de Pessoa Física",
            favorito: false,
            icone:
              "https://creci-hgtxcore-api.goutron.com.br/api/v1/image_render/app_ico/161",
            urlDestino:
              "https://creci-procinscr.goutron.com.br/auth/pessoa-fisica",
            nativoMobile: false,
            disponivel: true,
            indisponivelMensagem: "",
            appShareUrl:
              "https://creci-hgtxcore.goutron.com.br/app-share-redir?token=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..hN3z7GbREg6HBSZ1aPzyiJuIEiZDQLBCKpdOaYMLlh8",
          },
          {
            aplicativoID: 164,
            nome: "Gerenciar Inscrições de Pessoa Jurídica",
            favorito: false,
            icone:
              "https://creci-hgtxcore-api.goutron.com.br/api/v1/image_render/app_ico/164",
            urlDestino:
              "https://creci-procinscr.goutron.com.br/auth/pessoa-juridica",
            nativoMobile: false,
            disponivel: true,
            indisponivelMensagem: "",
            appShareUrl:
              "https://creci-hgtxcore.goutron.com.br/app-share-redir?token=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..I8XQ7jVrNY49LhJkfBePjsk-QRbM0nJHualZ_ChyARQ",
          },
          {
            aplicativoID: 159,
            nome: "Inscrição - Estagiário",
            favorito: false,
            icone:
              "https://creci-hgtxcore-api.goutron.com.br/api/v1/image_render/app_ico/159",
            urlDestino:
              "https://creci-procinscr.goutron.com.br/auth/inscricao-estagios",
            nativoMobile: false,
            disponivel: true,
            indisponivelMensagem: "",
            appShareUrl:
              "https://creci-hgtxcore.goutron.com.br/app-share-redir?token=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..RiwehGzxiwrp9DnNNj9FfuXXs1B-1h51mzyVmUTBkQ4",
          },
          {
            aplicativoID: 160,
            nome: "Inscrição - Pessoa Física",
            favorito: false,
            icone:
              "https://creci-hgtxcore-api.goutron.com.br/api/v1/image_render/app_ico/160",
            urlDestino:
              "https://creci-procinscr.goutron.com.br/auth/inscricao-pf",
            nativoMobile: false,
            disponivel: true,
            indisponivelMensagem: "",
            appShareUrl:
              "https://creci-hgtxcore.goutron.com.br/app-share-redir?token=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..e3c1iHMIUkx-ccF7WfEqiX9TBwhHNN4egX4XcbD6tbs",
          },
          {
            aplicativoID: 163,
            nome: "Inscrição - Pessoa Jurídica",
            favorito: false,
            icone:
              "https://creci-hgtxcore-api.goutron.com.br/api/v1/image_render/app_ico/163",
            urlDestino:
              "https://creci-procinscr.goutron.com.br/auth/inscricao-pj",
            nativoMobile: false,
            disponivel: true,
            indisponivelMensagem: "",
            appShareUrl:
              "https://creci-hgtxcore.goutron.com.br/app-share-redir?token=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..iRSatOR9hvqdG7ICIoZ9Ddo6hYKd6VRh2M8eWKo9zf0",
          },
          {
            aplicativoID: 157,
            nome: "Parâmetros Gerais",
            favorito: false,
            icone:
              "https://creci-hgtxcore-api.goutron.com.br/api/v1/image_render/app_ico/157",
            urlDestino:
              "https://creci-procinscr.goutron.com.br/auth/parametros",
            nativoMobile: false,
            disponivel: true,
            indisponivelMensagem: "",
            appShareUrl:
              "https://creci-hgtxcore.goutron.com.br/app-share-redir?token=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.._jl-cXL2wNKBV2VfHLPHyMBcMCIqgBU_JD5SoYzKHFE",
          },
          {
            aplicativoID: 19,
            nome: "Processo Inscricionário",
            favorito: false,
            icone:
              "https://creci-hgtxcore-api.goutron.com.br/api/v1/image_render/app_ico/19",
            urlDestino:
              "https://creci-procinscr.goutron.com.br/auth/requerimentos",
            nativoMobile: false,
            disponivel: true,
            indisponivelMensagem: "",
            appShareUrl:
              "https://creci-hgtxcore.goutron.com.br/app-share-redir?token=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..WFkEYoq73gPa95ANTtlRxzhhRLwwqGnb8Dq5oixxseE",
          },
        ],
        errorCode: 0,
        errorMessage: "",
      },
    }).as("getApps");

    cy.contains("Processo Inscricionário").click();
  });

  it.skip("deve ser capaz de visualizar a página de Consultar Censo - Pessoa Física.", () => {
    cy.origin("https://creci-procinscr.goutron.com.br/auth/censo-pf", () => {
      cy.contains("h2", "Consultar Censo - Pessoa Física").should("be.visible");
    });
  });

  it.skip("deve ser capaz de visualizar um grid listando censos.", () => {
    cy.origin("https://creci-procinscr.goutron.com.br/auth/censo-pf", () => {
      cy.viewport(1920, 1080);

      cy.log("Verificando a ordem das colunas");
      cy.get("div[role='rowgroup']")
        .should("be.visible")
        .find("div[role='columnheader']")
        .then(($cols) => {
          const titles = [...$cols].map((c) => c.innerText.trim());

          expect(titles).to.deep.equal([
            "ID",
            "Data do preenchimento",
            "Nome",
            "Detalhes",
          ]);
        });
    });
  });

  it.skip("deve ser capaz de visualizar uma mensagem de erro na obtenção dos resultados do grid listando requerimentos.", () => {});

  it.skip("deve ser capaz de clicar em um descritivo no canto superior direito abaixo do título Processo Inscricionário e abrir uma aba para a página da HGTX para esclarecimento de dúvidas.", () => {});

  // ----------------------------ver detalhes-------------------------------

  it.skip("deve ser capaz de visualizar uma modal com informações detalhadas do censo.", () => {});

  it.skip("deve ser capaz de visualizar uma mensagem de erro ao tentar obter as informações detalhadas do censo, pois o servidor está indisponível.", () => {});

  it.skip("deve ser capaz de visualizar informações detalhadas do censo sobre Dados Cadastrais.", () => {});

  it.skip("deve ser capaz de visualizar informações detalhadas do censo sobre Dados para Contato.", () => {});

  it.skip("deve ser capaz de visualizar informações detalhadas do censo sobre Dados Socioeconômicos.", () => {});

  it.skip("deve ser capaz de visualizar informações detalhadas do censo sobre Perfil Sociocultural.", () => {});

  it.skip("deve ser capaz de fechar a modal de detalhes do ceso clicando no botão Voltar..", () => {});
});
